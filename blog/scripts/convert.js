...
const SITE_URL = "https://confined-space.com";
...
function buildPostPage(title, dateDisplay, isoDate, bodyHtml, imagePath, slug) {
  const template = fs.readFileSync(TEMPLATE_FILE, "utf8");
  const shareImageUrl = imagePath ? SITE_URL + "/blog/" + imagePath : SITE_URL + "/images/cst-logo.png";
    ...
    .reduce((chain, filename) => {
      return chain.then(() => {
        ...
        return convertFile(filePath, filename).then(({ blocks, image }) => {
          ...
          const pageHtml = buildPostPage(title, dateDisplay, isoDate, bodyHtml, imagePath, slug);
